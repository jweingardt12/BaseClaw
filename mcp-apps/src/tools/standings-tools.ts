...
export function registerStandingsTools(server: McpServer, distDir: string, enabledTools?: Set<string>) {
  ...
  defineTool(
    ...
    async () => {
      ...
      if (snapshot && snapshot.teams && snapshot.teams.length > 0) {
        ...
        return {
          text: lines.join("\n"),
          structured: { type: "standings", ai_recommendation, ...snapshot },
        };
      }

      // Fallback: basic standings
      var data = await apiGet<StandingsResponse>("/api/standings");
      var text = "League Standings:\n" + data.standings.map((s) =>
        "  " + String(s.rank).padStart(2) + ". " + str(s.name).padEnd(30) + tkey(s.team_key) + " " + s.wins + "-" + s.losses
        + (s.points_for ? " (" + s.points_for + " pts)" : "")
      ).join("\n");
      var ai_rec = generateStandingsInsight(data);
      return {
        text,
        structured: { type: "standings", ai_recommendation: ai_rec, ...data },
      };
    },
    enabledTools,
  );
  ...
  defineTool(
    ...
    async ({ week }) => {
      ...
      if (week) params.week = week as string;
      var data = await apiGet<MatchupsResponse>("/api/matchups", params);
      var weekLabel = (week as string) || "current";
      var text = "Matchups (week " + weekLabel + "):\n" + data.matchups.map((m) =>
      ...
      return {
        text,
        structured: { type: "matchups", ai_recommendation, ...data },
      };
    },
    enabledTools,
  );
  ...
  defineTool(
    ...
    async () => {
      var data = await apiGet<MatchupDetailResponse>("/api/matchup-detail");
      ...
      return {
        text: lines.join("\n") + footer,
        structured: { type: "matchup-detail", ai_recommendation, ...data },
      };
    },
    enabledTools,
  );
  ...
  defineTool(
    ...
    async () => {
      var data = await apiGet<LeagueContextResponse>("/api/league-context");

      var lines: string[] = ["LEAGUE CONTEXT:"];
      ...
      var batCats = (data.stat_categories || [])
        .filter((c) => c.position_type === "B")
        .map((c) => str(c.name));
      var pitCats = (data.stat_categories || [])
  ...
  defineTool(
    ...
    async ({ trans_type, count, limit, offset }) => {
      var params: Record<string, string> = {};
      if (trans_type) params.type = trans_type as string;
      params.count = String(count);
      params.limit = String(limit);
      params.offset = String(offset);
      var data = await apiGet<TransactionsResponse>("/api/transactions", params);
      var label = (trans_type as string) || "all";
      var text = "Recent transactions (" + label + "):\n" + data.transactions.map((t) =>
      ...
      return {
        text,
        structured: { type: "transactions", ai_recommendation, ...data },
      };
    },
    enabledTools,
  );
  ...
  defineTool(
    ...
    async ({ limit, offset }) => {
      var data = await apiGet<TransactionTrendsResponse>("/api/transaction-trends", { limit: String(limit), offset: String(offset) });
      var addedLines = (data.most_added || []).slice(0, 10).map((p, i) =>
        "  " + String(i + 1).padStart(2) + ". " + str(p.name).padEnd(25) + " " + str(p.team).padEnd(4)
        + " " + str(p.percent_owned) + "% (" + str(p.delta) + ")"
      );
      var droppedLines = (data.most_dropped || []).slice(0, 10).map((p, i) =>
        "  " + String(i + 1).padStart(2) + ". " + str(p.name).padEnd(25) + " " + str(p.team).padEnd(4)
        + " " + str(p.percent_owned) + "% (" + str(p.delta) + ")"
      );
      var text = "Most Added:\n" + addedLines.join("\n") + "\n\nMost Dropped:\n" + droppedLines.join("\n");
      ...
      return {
        text,
        structured: { type: "transaction-trends", ai_recommendation, ...data },
      };
    },
    enabledTools,
  );
  ...
  defineTool(
    ...
    async () => {
      var data = await apiGet<LeaguePulseResponse>("/api/league-pulse");
      var lines = [
      ...
      return {
        text: lines.join("\n"),
        structured: { type: "league-pulse", ai_recommendation, ...data },
      };
    },
    enabledTools,
  );
  ...
  defineTool(
    ...
    async () => {
      var data = await apiGet<LeagueIntelResponse>("/api/league-intel");
      var lines = [
      ...
      for (var r of data.power_rankings) {
        var marker = r.is_my_team ? " <-- YOU" : "";
        var upside = r.z_upside || 0;
        var upsideStr = (upside > 0 ? "+" : "") + String(upside);
      ...
      var myTeam = data.power_rankings.find(r => r.is_my_team);
      var ai_recommendation = myTeam
      ...
      return {
        text: lines.join("\n"),
        structured: { type: "power-rankings", ai_recommendation, ...data },
      };
    },
    enabledTools,
  );
  ...
  defineTool(
    ...
    async () => {
      ...
      for (var i = 0; i < Math.min(data.top_performers.length, 20); i++) {
        var p = data.top_performers[i];
        var flags: string[] = [];
      ...
      lines.push("\nTEAM PROFILES:");
      for (var tp of data.team_profiles) {
        var tpMarker = tp.is_my_team ? " (YOU)" : "";
        var tpUpside = tp.z_upside || 0;
        var tpUpsideStr = (tpUpside > 0 ? "+" : "") + String(tpUpside);
        lines.push("\n  #" + tp.rank + " " + str(tp.name) + tpMarker + " | " + tp.record
          + " | Adj Z: " + tp.adjusted_z_total + " | Upside: " + tpUpsideStr);
        if (tp.top_players.length > 0) {
          lines.push("    Stars: " + tp.top_players.join(", "));
        }
        // Quality + signals
        var qParts: string[] = [];
        for (var tier of ["elite", "strong", "average"]) {
          var cnt = tp.quality_breakdown?.[tier];
      ...
      if ((data.category_leaderboards || []).length > 0) {
        ...
        for (var lb of data.category_leaderboards) {
          var top = (lb.rankings || [])[0];
          if (top) {
            var lbMarker = top.is_my_team ? " *" : "";
            lines.push("  " + str(lb.category).padEnd(10)
              + str(top.team_name).slice(0, 28).padEnd(28)
              + String(top.value).padStart(10) + lbMarker);
          }
        }
      }
      ...
      if ((data.h2h_matrix || []).length > 0) {
        ...
        lines.push("  " + "-".repeat(48));
        for (var entry of data.h2h_matrix) {
          var overall = entry.overall || { wins: 0, losses: 0, ties: 0 };
          var record = overall.wins + "-" + overall.losses + "-" + overall.ties;
          var h2hMarker = entry.is_my_team ? " <-- YOU" : "";
          lines.push("  " + str(entry.team_name).slice(0, 28).padEnd(28)
            + record.padStart(10) + "  " + str(entry.streak).padStart(4) + h2hMarker);
        }
      }

      // Build AI recommendation
      var myTeam = data.power_rankings.find(r => r.is_my_team);
      var rivals = data.power_rankings.filter(r => !r.is_my_team).slice(0, 3);
      var tradeFits = data.team_profiles.filter(tp => !tp.is_my_team && tp.trade_fit).slice(0, 2);
      var upsideTeams = data.team_profiles.filter(tp => !tp.is_my_team && tp.z_upside > 5).sort((a, b) => b.z_upside - a.z_upside).slice(0, 2);
      var paperTigers = data.team_profiles.filter(tp => !tp.is_my_team && tp.z_upside < -5).sort((a, b) => a.z_upside - b.z_upside).slice(0, 2);
      ...
      if ((data.category_leaderboards || []).length > 0) {
        var myLeads: string[] = [];
        var myTrails: string[] = [];
        for (var lb2 of data.category_leaderboards) {
          var myRank = (lb2.rankings || []).find(r => r.is_my_team);
          if (myRank) {
            if (myRank.rank === 1) myLeads.push(lb2.category);
            else if (myRank.rank >= (lb2.rankings || []).length - 1) myTrails.push(lb2.category);
          }
        }
      ...
      // H2H record insights
      if ((data.h2h_matrix || []).length > 0) {
        var myH2H = data.h2h_matrix.find(e => e.is_my_team);
        if (myH2H) {
          var o = myH2H.overall || { wins: 0, losses: 0, ties: 0 };
          ai_recommendation += "H2H record: " + o.wins + "-" + o.losses + "-" + o.ties
            + (myH2H.streak ? " (" + myH2H.streak + ")" : "") + ". ";
          var noWinsAgainst = (myH2H.vs || []).filter(v => v.wins === 0 && (v.losses > 0 || v.ties > 0));
  ...
  defineTool(
    ...
    async () => {
      var data = await apiGet<SeasonPaceResponse>("/api/season-pace");
      var lines = [
      ...
      for (var t of data.teams) {
        var record = t.wins + "-" + t.losses;
        if (t.ties) record += "-" + t.ties;
        var marker = t.is_my_team ? " <-- YOU" : "";
      ...
      var ai_recommendation = generateSeasonPaceInsight(data);
      return {
        text: lines.join("\n"),
        structured: { type: "season-pace", ai_recommendation, ...data },
      };
    },
    enabledTools,
  );
  ...
  defineTool(
    ...
    async () => {
      var data = await apiGet<PositionalRanksResponse>("/api/positional-ranks");
      var lines = ["League Positional Rankings:"];
      for (var team of data.teams || []) {
        lines.push("\n" + str(team.name) + tkey(team.team_key) + ":");
        for (var pr of team.positional_ranks || []) {
          var grade = pr.grade === "strong" ? "+" : pr.grade === "weak" ? "-" : " ";
          var starters = (pr.starters || []).map((p) => p.name + " (" + p.player_key + ")").join(", ");
      ...
      var ai_recommendation = "Review positional ranks to identify where your team is weak and find trade partners who are strong in those positions.";
      return {
        text: lines.join("\n"),
        structured: { ai_recommendation, ...data },
      };
  ...
  defineTool(
    ...
    async () => {
      ...
      if (!matchup) {
        return { text: "No active matchup found.", structured: { type: "redzone", ...data } };
      }

      var myTeam = data.teams[matchup.my_team_id];
      var oppTeam = data.teams[matchup.opponent_id];
      if (!myTeam || !oppTeam) {
        return { text: "Could not find matchup teams in redzone data.", structured: { type: "redzone", ...data } };
      }
  ...
  defineTool(
    ...
    "yahoo_competitor_tracker",
    {
      description: "Track what rival managers are doing — roster moves, category impact, sniped targets, and threat levels for each competitor",
      annotations: READ_ANNO,
      _meta: {},
    },
    async () => {
      ...
      return {
        text: lines.join("\n"),
        structured: { type: "competitor-tracker", ...data },
      };
    },
    enabledTools,
  );
  ...
  defineTool(
    ...
    "yahoo_watchlist_add",
    {
      description: "Add a player to your watchlist for tracking ownership changes, z-score movement, and news alerts",
      inputSchema: {
        name: z.string().describe("Player name to watch"),
        reason: z.string().optional().describe("Why you're watching this player"),
        type: z.enum(["pickup", "trade_target", "monitor", "sell_candidate"]).optional().describe("Tracking type"),
      },
      annotations: WRITE_ANNO,
      _meta: {},
    },
    async ({ name, reason, type }) => {
      var data = await apiPost<any>("/api/watchlist-add", { name: name as string, reason: (reason as string) || "", type: (type as string) || "monitor" });
      return { text: "Added " + data.added + " to watchlist (" + (data.type || "monitor") + "). Owner: " + (data.owner || "free agent") + ". Z-score: " + data.z_score };
    },
    enabledTools,
  );
  ...
  defineTool(
    ...
    "yahoo_watchlist_remove",
    {
      ...
      inputSchema: { name: z.string().describe("Player name to remove") },
      annotations: WRITE_ANNO,
      _meta: {},
    },
    async ({ name }) => {
      var data = await apiPost<any>("/api/watchlist-remove", { name: name as string });
      return { text: "Removed " + data.removed + " from watchlist" };
    },
    enabledTools,
  );
  ...
  defineTool(
    ...
    "yahoo_watchlist",
    {
      description: "Check all watched players for ownership changes, z-score movement, and status updates",
      annotations: READ_ANNO,
      _meta: {},
    },
    async () => {
      ...
      return {
        text: lines.join("\n"),
        structured: { type: "watchlist", ...data },
      };
    },
    enabledTools,
  );
  ...
  defineTool(
    ...
    "yahoo_category_arms_race",
    {
      description: "Show category-by-category competitive position — your rank, gap to nearest rivals, and who's gaining on you",
      annotations: READ_ANNO,
      _meta: {},
    },
    async () => {
      ...
      return {
        text: lines.join("\n"),
        structured: { type: "category-arms-race", ...data },
      };
    },
    enabledTools,
  );
  ...
  defineTool(
    ...
    "yahoo_research_feed",
    {
      description: "Unified intelligence feed — news, league transactions, trending pickups, prospect signals. Filter by: roster, league, market, prospects, all",
      inputSchema: {
        filter: z.enum(["all", "roster", "league", "market", "prospects", "news"]).optional().describe("Feed filter").default("all"),
    ...
    async ({ filter, limit }) => {
      var data = await apiGet<any>("/api/research-feed?filter=" + ((filter as string) || "all") + "&limit=" + ((limit as number) || 20));
      ...
      return {
        text: lines.join("\n"),
        structured: { type: "research-feed", ...data },
      };
    },
    enabledTools,
  );
